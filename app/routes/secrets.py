...
def list_secrets():
    ...
    slugs = VaultClient.kv_list(path=VAULT_SECRETS_PATH + "/")
    results = []

    for slug in slugs:
        slug_clean = slug.rstrip("/")
        data = VaultClient.kv_read(f"{VAULT_SECRETS_PATH}/{slug_clean}")
        if data is None:
            continue
        if tag_filter:
...
def get_secret(slug: str):
    ...
    data = VaultClient.kv_read(f"{VAULT_SECRETS_PATH}/{slug}")
    if data is None:
...
def create_secret():
    ...
    existing = VaultClient.kv_read(f"{VAULT_SECRETS_PATH}/{slug}")
    if existing:
    ...
    VaultClient.kv_write(f"{VAULT_SECRETS_PATH}/{slug}", secret_data)
...
def delete_secret(slug: str):
    ...
    ok = VaultClient.kv_delete(f"{VAULT_SECRETS_PATH}/{slug}")
    if not ok:
